 ...
 * @type {(arg0: any) => any}
 */
export const reduceToSix = (string_number) => {
    //const digits = convertToDigits2(string_number);
...
export const reduceToSixLoop = (/** @type {string | any[]} */ n)=> {
    // console.log(n);
    let n1 = n;
    const final = n.length - 1;
...
export const reduceToSixArray = (n) =>{
    //console.log(n);
    const a = [];
    let n1 = n;
    const final = n.length - 1;
...
export const calculateEquis = (x) => {
    // final index
    const final = x.length - 1;
    const ntop = parseInt(x[final]);
    const nrigth = (parseInt(x[final -1])%10 + parseInt(x[final]) )%10;
    const nleft = (Math.floor(parseInt(x[final - 1])/10) + parseInt(x[final]))%10;
    const nbotton = (nleft + nrigth)%10;
...
export const calculateEquisValues = (x)=> {
    // final index
    const final = x.length - 1;
    const ntop = parseInt(x[final]);
    const nrigth = (parseInt(x[final -1])%10 + parseInt(x[final]) )%10;
...
export const lotteryNumber = (/** @type {any} */ seed) => {
